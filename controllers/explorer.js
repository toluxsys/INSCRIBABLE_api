...
module.exports.getJsonInscription = async (req, res) => {
  try {
    ...
    if (!result)
      return res
        .status(200)
        .json({ status: false, message: 'inscriptions not found' });
    return res
      .status(200)
      .json({ status: true, message: 'ok', userResponse: result });
  } catch (e) {
...
module.exports.getInscriptionContent = async (req, res) => {
  try {
    ...
    if (!result)
      return res
        .status(200)
        .json({ status: false, message: 'inscriptions content not found' });
    return res
      .status(200)
      .json({ status: true, message: 'ok', userResponse: result });
  } catch (e) {
...
module.exports.getInscriptionFeed = async (req, res) => {
  try {
    ...
    if (!result)
      return res
        .status(200)
        .json({ status: false, message: 'unable to load feed' });
    return res
      .status(200)
      .json({ status: true, message: 'ok', userResponse: result });
  } catch (e) {
